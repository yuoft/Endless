...
public class InfinityHoe extends HoeItem {
    ...
    public InteractionResult useOn(UseOnContext context) {
        ...
        if (pair == null) return InteractionResult.FAIL;
            ...
            if (!world.isClientSide && player != null) {
                ...
                if (player.isCrouching()){
                    ...
                    for (BlockPos pos : boxMutable) {
                        ...
                        if (state.liquid() || state.getBlock() instanceof LiquidBlockContainer){ //液体或可替换变为耕地
                            world.setBlock(pos, blockstate, 11);
                        }
                    }
                    ...
                    for (BlockPos pos: allInBoxMutable){
                        if (!hasBox(pos, inBoxMutable)){ //外壳坐标
                            BlockState state = world.getBlockState(pos);
                            if (state.liquid() || state.getBlock() instanceof LiquidBlockContainer)//如果是液体，则将其填埋
