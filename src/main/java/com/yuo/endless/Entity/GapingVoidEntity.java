...
//        ignoreFrustumCheck = true;
    ...
    public void tick() {
        ...
        if (age >= maxLifetime && !level.isClientSide) { //死亡时发生爆炸
            level.explode(this, posX, posY, posZ, 6.0f, true, ExplosionInteraction.BLOCK);
            int range = 4;
            ...
            for (Entity nommee : nommed) {
                if (nommee != this) {
                    if (nommee instanceof EnderDragon dragon){
                        dragon.hurt(dragon.head, this.damageSources().explosion(useEntity, dragon), endDamage);
                    }else if (nommee instanceof WitherBoss wither){
                        wither.setInvulnerableTicks(0);
                        wither.hurt(this.damageSources().explosion(useEntity, wither), endDamage);
                    } else nommee.hurt(this.damageSources().explosion(useEntity, nommee), endDamage);
                }
            }
            discard();
        } else {
        ...
        if (age % 5 == 0){ //每5tick攻击一次 一秒4次
            AABB alignedBB = new AABB(vec3.add(-nomRange, -nomRange, -nomRange), vec3.add(nomRange,nomRange,nomRange));
            ...
            for (LivingEntity living : livings) {
                ...
                if (len <= nomRange) {
                    if (living instanceof EnderDragon dragon){
                        dragon.hurt(dragon.head, this.damageSources().fellOutOfWorld(), oneDamage);
                    }
                    else living.hurt(this.damageSources().fellOutOfWorld(), oneDamage);
