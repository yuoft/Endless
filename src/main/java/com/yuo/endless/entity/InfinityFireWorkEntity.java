    ...
    private void dealExplosionDamage() {
        ...
        if (f > 0.0F) {
            if (this.attachedToEntity != null) {
                this.attachedToEntity.hurt(this.damageSources().fireworks(this, this.getOwner()), this.damage + (float)(listnbt.size() * 2));
            }
            ...
            for(LivingEntity livingentity : this.level.getEntitiesOfClass(LivingEntity.class, this.getBoundingBox().deflate(d0))) {
                if (livingentity != this.attachedToEntity && !(this.distanceToSqr(livingentity) > 25.0D)) {
                    ...
                    if (flag) { //伤害衰减
                        ...
                        livingentity.hurt(damageSources().fireworks(this, this.getOwner()), f1);
                    }
                }
            }
        }
