...
public class InfinityBoxTile extends AbsEndlessChestTile implements RecipeHolder {
        ...
        if (tile.isBurning() || !burnFuel.isEmpty() && !burnItem.isEmpty()){
            ...
            if (optional.isPresent()){
                ...
                if (!tile.isBurning() && tile.canSmelt(recipe)){ //增加燃烧时间
                    ...
                    if (tile.isBurning()) {
                        flag1 = true;
                        if (burnFuel.hasCraftingRemainingItem())
                            tile.items.set(254, burnFuel.getCraftingRemainingItem());
                        else
                        if (!burnFuel.isEmpty()) {
                            burnFuel.shrink(1);
                            if (burnFuel.isEmpty()) {
    ...
    private boolean canSmelt(@Nullable SmeltingRecipe recipeIn) {
        ...
        if (!burnItem.isEmpty() && recipeIn != null) {
            ItemStack itemstack = recipeIn.assemble(new SimpleContainer(burnItem), RegistryAccess.EMPTY);
            if (itemstack.isEmpty()) {
                ...
                if (burnResult.isEmpty()) {
                    return true;
                } else if (!ItemStack.isSameItem(burnResult, itemstack)) {
                    return false;
    ...
    private void smelt(@Nullable SmeltingRecipe recipe, Level world) {
        if (recipe != null && this.canSmelt(recipe)) {
            ItemStack burnItem = this.items.get(253);
            ItemStack craftingResult = recipe.assemble(new SimpleContainer(burnItem), RegistryAccess.EMPTY);
            ItemStack burnResult = this.items.get(255);
            if (burnResult.isEmpty()) {
    ...
    public Component getDisplayName() {
        return Component.translatable("gui.endless.infinity_chest");
    }
    ...
    public void NbtRead(CompoundTag nbt){
        ...
        for(String s : compoundnbt.getAllKeys()) {
            this.recipes.put(EndlessUtils.parse(s), compoundnbt.getInt(s));
        }
    }
